...
O painel pede senha ao abrir `admin.html`.
...
Para Railway, coloque `catalog-seed.json` na raiz antes do primeiro deploy. O seed so e aplicado se o banco estiver vazio.
...
Em desenvolvimento sem `DATABASE_URL`, o servidor salva em `.catalog-local.json`. Em producao com `NODE_ENV=production`, `DATABASE_URL` e `ADMIN_PASSWORD` sao obrigatorios.
...
4. Abrir `/api/health`.
5. Acessar `/admin.html`, cadastrar um produto e conferir na loja.
